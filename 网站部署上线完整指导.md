...
1. **一键发版脚本**：每次发布前运行以下命令（需 Node.js 18+）：
   ...
   - 为 CSS/JS/配置脚本追加 `?v=版本号`（index.html、collection.html、privacy-policy.html、terms-of-service.html、contact.html）
   ...
   执行完成后，提交并推送：
   ```bash
   git add sw.js index.html collection.html privacy-policy.html terms-of-service.html contact.html sitemap*.xml robots.txt offline.html assets/js/swRegister.js tools/release.mjs
