...
1. 使用全自动发布脚本（需 Node.js 18+）：
   ```bash
   # 在项目根目录（可选 --git 自动提交并尝试 push）
   node tools/publish.mjs --version 1.0.3 --site https://minigameshub.co --git
   ```
   该脚本会自动：
   - 重新生成静态页面（generated-pages/，基于 templates/ 与 data/gameData.json）
   - 调用 tools/release.mjs：升级 Service Worker 版本、为核心 HTML 注入 `?v=版本号`、重建站点地图、更新 robots.txt 与首页 JSON-LD 计数
